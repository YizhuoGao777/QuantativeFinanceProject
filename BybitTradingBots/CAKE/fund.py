...
# === 1. 读取 CSV ===
df = pd.read_csv("CAKElog2025.10.csv", header=None)
...
plt.plot(df_plot["time"], df_plot["funds"], color="steelblue", linewidth=1.8, label="资金曲线（奇数行）")
plt.fill_between(df_plot["time"], df_plot["funds"], df_plot["peak"], color="red", alpha=0.1, label="回撤区域")

plt.scatter(df_plot["time"].iloc[start_idx], df_plot["funds"].iloc[start_idx], color="green", label="峰值点")
plt.scatter(df_plot["time"].iloc[end_idx], df_plot["funds"].iloc[end_idx], color="red", label="谷值点")
plt.plot(
