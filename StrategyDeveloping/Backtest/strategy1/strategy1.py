...
class Strategy1(StrategyBase):
        ...
        if not self.active:
            ...
            if self.window.__len__() < 15:
                ...
                if self.positions.__len__() == 0:
                    if  float(data["资金费率"][:-1]) > 0.005 and self.ifLiquidationEnds("short") == True :  ###float(data["资金费率"][:-2]) > 0.005 and   and data["Long Liquidation(1h)"][-1] != "M"
                        newPosition = {"time":data["time"], "action": "short", "position": 0.03,
                    ...
                    if self.ifLiquidationEnds("long") == True :
                        ...
                        newPosition = {"time": data["time"], "action": "long",
                                       "position": 0.03,
                                       "value": 0.03 * float(data["价格"][1:]), "entryprice": data["价格"]}
                ...
                ## #触发止损时卖出
                        ...
                        if (float(data["价格"][1:]) - float(self.positions[0]["entryprice"][1:])) / float(self.positions[0]["entryprice"][1:]) > 0.01  :
                            self.update_pnl(data)
                            newPosition = {"time": data["time"], "action": "stoplosing",
                                           "position": 0.03,
                                           "value": 0.03 * float(data["价格"][1:]), "entryprice": data["价格"]}
    ...
    def ifLiquidationEnds(self, liqudationSide):  ##只写了爆仓逐渐减少。下一步完善成先增加后减少.改成先增加后减少后效果更差
        if liqudationSide == "short":
            for i in range(self.window.__len__() - 1):
                ...
                if data1[-1] != "M" or data1.lstrip("$").rstrip("M")[:-1] < data2.lstrip("$").rstrip("M")[:-1]: ##比较爆仓数值2.1 《 2.2
                    return False
            ###########
           # if self.window[-1]["Short Liquidation(1h)"][-1] == "M": return False
            return True
