...
def main():
    ...
    with (open(f'../../数据/CAKE/{symbol}_1m_{period}.csv', newline='', encoding='utf-8') as csvfile1,
          open(f'../../数据/CAKE/{symbol}_15m_{period}.csv', newline='', encoding='utf-8') as csvfile2):
        ...
        with Live(makeTable(i), refresh_per_second=1) as live:
            for row1 in reader1:
                if i == 1000:
                    print(".")
                if i in range(15,130000): #只处理range里面的row
